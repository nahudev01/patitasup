          ...
          <section className="flex-1 p-6 md:p-8">
            <div className="grid gap-6 xl:grid-cols-[minmax(0,2fr)_minmax(280px,1fr)]">
              <article className="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-black/5 md:p-8">
                ...
                <h1 className="mt-4 text-[30px] font-semibold text-[#1f2937]">
                  Hola, {profileName}
                </h1>

                <p className="mt-3 max-w-2xl text-[15px] leading-7 text-[#6b7280]">
                  Este espacio ya está autenticado con Supabase SSR y sincronizado con Prisma.
                  Desde acá podés seguir evolucionando el panel de rescatistas sobre una sesión
                  persistente y segura para Vercel.
                </p>
              ...
              <aside className="rounded-3xl bg-[#0B1120] p-6 text-white shadow-sm">
                <p className="text-sm font-semibold uppercase tracking-[0.12em] text-white/60">
                  Sincronización Prisma
                </p>

                <div className="mt-5 space-y-4 text-sm text-white/80">
                  <p>
                    Perfil persistido: {profile ? "Sí" : "No"}
                  </p>
                  <p>
                    Nombre guardado: {profile?.displayName ?? profileName}
                  </p>
                  <p>
                    Email guardado: {profile?.email ?? user.email ?? "Sin correo"}
                  </p>
                </div>
              </aside>
            </div>
...
function InfoCard({ label, value, mono = false }: InfoCardProps) {
  return (
    <div className="rounded-2xl border border-[#ececf2] bg-[#fafbff] p-4">
      ...
      <p className={`mt-3 text-[15px] text-[#111827] ${mono ? "font-mono text-[13px]" : ""}`}>
        {value}
      </p>
    </div>
  );
